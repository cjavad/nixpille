...
function _bw_json -a json_input filter
    ...
    echo "$json_input" | jq -r "$filter" 2>/dev/null
end
...
function _bw_get_item_id -a name -a type
    bw_session || return 1
    set -l items (_bw_run list items 2>/dev/null)
    test -z "$items" && return 1

    set -l filter ".[] | select(.name == \"$name\")"
    test -n "$type" && set filter "$filter | select(.type == $type)"
    set filter "$filter | .id"

    set -l ids (_bw_json "$items" "$filter")
    test -z "$ids" && return 1
    echo $ids[1]
end
...
function bw_get_ssh -a id
    bw_session || return 1
    set -l item (_bw_run get item "$id" 2>/dev/null)
    _bw_json "$item" '.sshKey.privateKey // empty'
end
